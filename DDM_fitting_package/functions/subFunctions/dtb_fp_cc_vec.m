function P = dtb_fp_cc_vec(drift,t,Bup,Blo,y,y0,notabs_flag,vd_flag,varargin)
...
P.up.pdf_aboveZero = zeros(nd,nt) ;
P.lo.pdf_belowZero = zeros(nd,nt) ;

% if vd_flag
% P.up.pdf_aboveZero = zeros(nd,nt) ;
% P.up.p4VD = zeros(nd,nt) ;
% end
...
for k = 2:nt
    ...
    if vd_flag
        % Get density that's above zero
        P.up.pdf_aboveZero(:,k) = sum(ur(y>0,:),1) ;
        P.lo.pdf_belowZero(:,k) = sum(ur(y<0,:),1) ;
%         P.up.p4VD(:,k) = sum(P.up.pdf_t,2) + P.up.pdf_aboveZero(:,k) ;
    end    
...
if vd_flag
    P.up.p4VD = P.up.pdf_aboveZero + P.up.cdf_t ;
    P.up.p4VD(:,1) = .5 ;
    
    P.lo.p4VD = P.lo.pdf_belowZero + P.lo.cdf_t ;
    P.lo.p4VD(:,1) = .5 ;
end
